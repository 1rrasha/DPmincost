...
//	O(m), where m is the number of cities in the file.
	...
	private void handleFindMinCost() {
		...
		try {
			...
			// Print the selected cities and their corresponding indices
			System.out.println("Start City: " + Start + ", Index: " + StartCity);
			System.out.println("End City: " + Goal + ", Index: " + EndCity);

			// Check if the same city is selected for both start and end
			if (StartCity == EndCity) {
				// Show an alert for invalid selection and return
				showAlert("Invalid Selection", "Invalid Selection",
						"Please select different cities for start and end.");
				return;
			}

			// Ensure that EndCity is greater than or equal to StartCity
			if (EndCity >= StartCity) {
				...
				showAlert("Invalid Input", "Invalid Selection", "Please select valid start and end cities.");
			}
		} catch (NegativeArraySizeException e) {
			showAlert("Calculation Error", "An error occurred during calculation",
...
//	prints the path from source to destination city. It iterates over the next array to construct the path.
...
//in worst case it can reach O(2^n)
//	prints alternative paths between cities. 
...
//	helper method 
	...
	// number of citiest initializes the table and next arrays, reads city data from
	// the input string, and calculates the minimum cost using dynamic programming.
