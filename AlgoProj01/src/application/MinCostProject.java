 ...
 * MinCostProject:
 * 
 * By:Rasha Mansour -1210773 A JavaFX application for calculating minimum
 * transportation costs and finding shortest paths between cities. It reads city
 * costs from a file, applies dynamic programming algorithms, and provides an
 * intuitive user interface for analysis.
 */

public class MinCostProject extends Application {
...
//		hbPath.setAlignment(Pos.CENTER);
		...
		vbTable.setBackground(new Background(new BackgroundFill(Color.BLACK, null, null)));
	...
	private void loadFile(Stage primaryStage) {
		...
		if (selectedFile != null) {
			try {
				...
				// Now proceed with reading the file and updating UI accordingly
	...
	// Method to handle finding the minimum cost and hb3 path
	private void handleFindMinCost() {
		if (EndCity >= StartCity) {
			applyAlgorithm(start, end, input);
			int minCost = table[0][CitiesNum - 1];
			String PathL = "";
			if (minCost != Integer.MAX_VALUE) {
				PathL = printPath(next, Start, next[0][CitiesNum - 1]) + Goal;
			}
			MinCostF.setText(Integer.toString(minCost));
			PathF.setText(PathL);
		} else {
			MinCostF.setText("Sorry, There is no direct way:(.");
			PathF.setText("Sorry, There is no direct way:(.");
		}
	}

	// Method to handle printing alternative paths
	private void handleAltPathsBtn() {
		if (EndCity >= StartCity && table[0][CitiesNum - 1] != Integer.MAX_VALUE) {
			AltPath.setText(printShortestPathsToDestination(table, city, Start, Goal));
		} else {
	...
	// Method to go back to the main scene
	private void goBackToMainScene(Stage primaryStage) {
		primaryStage.setScene(scene);
		DPtable.clear();
	}

	// Method to handle displaying the table
	...
	public static String printShortestPathsToDestination(int[][] table, String[] city, String source,
		...
		if (shortestDistance != Integer.MAX_VALUE) {
			...
			printAllShortestPathsHelper(table, city, StartCity, EndCity, currentPath, shortestPaths);
	...
	private static void printAllShortestPathsHelper(int[][] table, String[] city, int currentIndex,
			int destinationIndex, ArrayList<String> currentPath, ArrayList<Path> shortestPaths) {
	...
	// Method to apply the algorithm for finding minimum cost
